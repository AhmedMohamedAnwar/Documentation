...
- Each storage you buy for first time called row storage.
...
![[Fragmentation Lecture 15.png]]
The problem when you write file on disk then pause and write another disk this cause Seek time the time that make read file slower.

---
### Defragmentation
Solving Fragmentation problem by writting in other place till writting full file then write the full file in the disk.
### df + ls -i
```shell 
# give you Inode number of files 
ls -il 
# give you all partitions -h = human readable
df -h
# give Inode aviailable - used - free 
df -i 
```